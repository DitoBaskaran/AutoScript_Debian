...
echo
echo "$0 v.whatsapp.com"
echo
echo "Note: v.whatsapp.com payload is example."
echo
else
...
sed -i 's|export KEY_CITY="SanFrancisco"|export KEY_CITY="Indramayu"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_ORG="Fort-Funston"|export KEY_ORG="DitoBaskaran"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_EMAIL="me@myhost.mydomain"|export KEY_EMAIL="ahmadfauzanhabibi@yahoo.com"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_OU="MyOrganizationalUnit"|export KEY_OU="DitoBaskaran"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_NAME="EasyRSA"|export KEY_NAME="DitoBaskaran"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_OU=changeme|export KEY_OU=DitoBaskaran|' /etc/openvpn/easy-rsa/vars
...
useradd openvpn
echo "openvpn:DitoBaskaran" | chpasswd
clear
